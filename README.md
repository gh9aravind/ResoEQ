...
## Stage 2: real parametric EQ (biquad filters)

`CaptureService` no longer uses `DynamicsProcessing`. It now runs its own
`ParametricEqEngine` (see `ParametricEqEngine.kt`) directly on the captured
PCM samples - a cascade of peaking biquad filters, one per band per channel,
using the standard Audio EQ Cookbook formulas. Each band has a true center
frequency, gain, and Q (bandwidth) - this is the "real" parametric EQ.

`Q` defaults to 1.0 for every band and isn't exposed in the UI yet
(`CaptureService.updateBandQ(index, q)` is ready for a slider whenever you
want to add one).

## Stage 3: session tracking instead of capture (no echo possible)

The app's **Start** button now uses `SessionEqService` instead of
`CaptureService`. Instead of capturing system audio and replaying a
processed copy (which always risks the original still playing too - the
echo we kept fighting), this listens for the standard Android broadcast
(`AudioEffect.ACTION_OPEN_AUDIO_EFFECT_CONTROL_SESSION`) that cooperative
media apps send when they open a player session, and attaches
`DynamicsProcessing` directly to *that app's own session*. The EQ then runs
inside the source app's own audio pipeline - there's no second AudioTrack,
so there's structurally nothing to echo.

Trade-off: this only works with apps that actually send that broadcast.
Many standalone music players do. Some apps (YouTube, and apps using the
newer native AAudio API) open a session without broadcasting it, so this
service won't affect them. `CaptureService` (Stage 1/2) is still in the
project as a fallback for exactly those apps - not wired up to the UI
right now, but available to combine later.
